function [output] = RBESF_vs_LET_BWF(kernelName, filePaths, FuncVals, referenceAlpha, referenceBeta, survivingFraction)
...
%It's going to be a quadratic formula
log(survivingFraction)
...
for i = 1:size(GPUExperimentalData.SF,3)
    ...
    for j = 1:GPUExperimentalData.sizeDose(1,1,i)
        ...
        %Alright, we need to implement the RBE SF calculation here
        %RBE = top: sqrt (alpha_p^2 + 4 beta_p * d (alpha_p + beta_d * d)) - alpha_p / 2 beta_gamma d

        %Wait, this isn't RBE for a given surviving fraction. Hmm.
