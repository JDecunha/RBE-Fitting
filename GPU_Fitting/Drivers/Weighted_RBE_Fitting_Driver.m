...
CostFunc = @(x) GPUCostFunction_Weighted(x, experimentalData, penaltyWeight, cudaKernel, GPUBuffer, cudaPenaltyKernel, GPUBuffer2);
...
for i = 1:numCycles
    ...
    if gradientAssist == true   
        ...
        %Dynamically update the temperatures if it has been requested
        if dynamicTemp == true
            options.InitialTemperature = abs(initialGuess)*10; %Use the gradient descent solution to set the temperature for each item
        elseif isempty(temps) == false
            options.InitialTemperature = temps;
        end
    ...
    if numIterationsWithoutImprovement >= toleranceCycles
        ...
        status = "Stopped because tolerance hit.";
        break
    end
...
if i == numCycles
    status = "Stopped because max cycles hit.";
end

%Calculate the cost metrics
additionalMetrics = CostMetrics(Soln, experimentalData, penaltyWeight, cudaKernel, GPUBuffer, cudaPenaltyKernel, GPUBuffer2, experimentalData.EffectiveNMeasurements);

%Smush into output
output = {Soln, Cost, additionalMetrics, i, status};
